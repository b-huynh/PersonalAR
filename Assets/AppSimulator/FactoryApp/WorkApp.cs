...
public class WorkApp : App
{
    ...
    {
        if (!knownObjects.Contains(registration.className)) return;
        ...
        menu.transform.position = registration.position + new Vector3(-0.4f, 0.0f, 0.0f);

        GameObject tooltip = menu.transform.Find("LineTooltip").gameObject;
        tooltip.GetComponent<ToolTipConnector>().Target = registration.gameObject;
